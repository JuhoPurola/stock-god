...
CREATE INDEX idx_stocks_name ON stocks USING gin(to_tsvector('english', name));
...
CREATE UNIQUE INDEX idx_stock_prices_symbol_timestamp ON stock_prices(symbol, timestamp);
...
CREATE UNIQUE INDEX idx_portfolio_snapshots_portfolio_timestamp ON portfolio_snapshots(portfolio_id, timestamp);
...
CREATE INDEX idx_strategy_performance_portfolio_id ON strategy_performance(portfolio_id);
...
CREATE INDEX idx_api_rate_limits_window ON api_rate_limits(api_name, endpoint, window_end);
