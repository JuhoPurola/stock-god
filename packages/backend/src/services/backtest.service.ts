...
export class BacktestService {
  ...
  async runBacktest(
    ...
    options?: { skipDatabaseRecording?: boolean }
  ...
  private async generateSignals(
    ...
    const contextProvider = async (symbol: string): Promise<EvaluationContext> => {
      const symbolPrices = priceData.get(symbol);
      if (!symbolPrices) {
      ...
      const historicalPrices: PriceBar[] = lookbackDates
        .map((date) => {
