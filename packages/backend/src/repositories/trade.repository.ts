...
export class TradeRepository {
  ...
  async findById(id: string): Promise<Trade | null> {
    const result = await query<any>('SELECT * FROM trades WHERE id = $1', [id]);
   ...
   */
  async findByIdOrThrow(id: string): Promise<Trade> {
   ...
   */
  async findByBrokerOrderId(brokerOrderId: string): Promise<Trade | null> {
    const result = await query<any>(
      'SELECT * FROM trades WHERE broker_order_id = $1',
      [brokerOrderId]
    );
   ...
   * Get pending trades
   */
  async findPending(portfolioId?: string): Promise<Trade[]> {
