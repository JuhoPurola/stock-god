...
export class StrategyRepository {
   ...
   */
  async findByPortfolioId(portfolioId: string): Promise<Strategy[]> {
    const result = await query<any>(
      'SELECT * FROM strategies WHERE portfolio_id = $1 ORDER BY created_at DESC',
      [portfolioId]
    );
   ...
   */
  async findEnabledByPortfolioId(portfolioId: string): Promise<Strategy[]> {
    const result = await query<any>(
      'SELECT * FROM strategies WHERE portfolio_id = $1 AND enabled = true',
      [portfolioId]
    );
   ...
   */
  async toggleEnabled(id: string): Promise<Strategy> {
    const result = await query<any>(
      `UPDATE strategies
