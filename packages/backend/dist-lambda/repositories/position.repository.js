...
export class PositionRepository {
     ...
     */
    async upsert(portfolioId, symbol, quantity, averagePrice, currentPrice, client) {
        const marketValue = quantity * currentPrice;
        const costBasis = quantity * averagePrice;
        const unrealizedPnL = marketValue - costBasis;
        const unrealizedPnLPercent = costBasis > 0 ? (unrealizedPnL / costBasis) * 100 : 0;
        const queryFn = client ? client.query.bind(client) : query;
        const result = await queryFn(`INSERT INTO positions (
     ...
     */
    async findByPortfolioAndSymbol(portfolioId, symbol) {
        const result = await query('SELECT * FROM positions WHERE portfolio_id = $1 AND symbol = $2', [portfolioId, symbol]);
    ...
    async findByPortfolioId(portfolioId) {
        const result = await query('SELECT * FROM positions WHERE portfolio_id = $1 ORDER BY market_value DESC', [portfolioId]);
        return result.rows.map((row) => this.mapToPosition(row));
    }
    /**
     * Get positions with stock details
     */
    async findByPortfolioIdWithDetails(portfolioId) {
      ...
      ORDER BY p.market_value DESC`, [portfolioId]);
     ...
     */
    async updatePrices(portfolioId, symbol, currentPrice, client) {
        const queryFn = client ? client.query.bind(client) : query;
        const result = await queryFn(`UPDATE positions
       SET
         current_price = $1,
       ...
       RETURNING *`, [currentPrice, portfolioId, symbol]);
        return result.rows[0] ? this.mapToPosition(result.rows[0]) : null;
    }
    /**
     * Update position after trade
     */
    async updateAfterTrade(portfolioId, symbol, quantityChange, price, client) {
        const queryFn = client ? client.query.bind(client) : query;
        // Get current position
        const currentResult = await queryFn('SELECT * FROM positions WHERE portfolio_id = $1 AND symbol = $2', [portfolioId, symbol]);
        ...
        if (newQuantity === 0) {
            // Close position
            await queryFn('DELETE FROM positions WHERE portfolio_id = $1 AND symbol = $2', [portfolioId, symbol]);
            return null;
        }
        ...
        if (quantityChange > 0) {
            ...
            const totalCost = currentQuantity * currentAvgPrice + quantityChange * price;
     ...
     * Delete position
     */
    async delete(portfolioId, symbol) {
