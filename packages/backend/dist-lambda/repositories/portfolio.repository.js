...
export class PortfolioRepository {
    ...
    async create(userId, data) {
        const result = await query(`INSERT INTO portfolios (user_id, name, description, cash_balance, trading_mode)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [userId, data.name, data.description, data.initialCash, data.tradingMode]);
        if (!result.rows[0]) {
    ...
    async findByIdWithStats(id) {
      ...
      GROUP BY p.id`, [id]);
        ...
        const snapshotResult = await query(`SELECT daily_return, daily_return_percent
    ...
    async findByUserId(userId) {
        const result = await query('SELECT * FROM portfolios WHERE user_id = $1 ORDER BY created_at DESC', [userId]);
    ...
    async update(id, data) {
        ...
        values.push(id);
        const result = await query(`UPDATE portfolios
       SET ${updates.join(', ')}
       WHERE id = $${paramIndex}
       RETURNING *`, values);
        if (!result.rows[0]) {
     ...
     * Update cash balance
     */
    async updateCashBalance(id, amount, client) {
        const queryFn = client ? client.query.bind(client) : query;
        await queryFn('UPDATE portfolios SET cash_balance = cash_balance + $1 WHERE id = $2', [amount, id]);
    }
    /**
     * Delete portfolio
     */
