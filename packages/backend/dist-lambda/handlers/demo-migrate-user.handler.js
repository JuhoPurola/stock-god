    ...
    try {
        ...
        if (action === 'seed-prices') {
            ...
            for (const symbol of stocks) {
             ...
             ON CONFLICT (symbol, timestamp) DO NOTHING`, [symbol, date.toISOString(), open.toFixed(4), high.toFixed(4), low.toFixed(4), close.toFixed(4), volume]);
                    recordsAdded++;
                }
            }
        ...
        if (action === 'rebalance') {
            ...
            // Find portfolio
            const portfolioResult = await query(`SELECT p.id FROM portfolios p
         ...
         LIMIT 1`, [email]);
            ...
            await query('UPDATE portfolios SET cash_balance = 10000, updated_at = NOW() WHERE id = $1', [portfolioId]);
        ...
        const auth0User = await query('SELECT id, email FROM users WHERE email = $1 ORDER BY created_at DESC LIMIT 1', [email]);
        let userId;
        if (auth0User.rows.length === 0) {
            ...
            const createResult = await query('INSERT INTO users (id, email, name, created_at, updated_at) VALUES ($1, $2, $3, NOW(), NOW()) ON CONFLICT (id) DO NOTHING RETURNING id', [auth0Id, email, 'User']);
        ...
        const existingPortfolio = await query(`SELECT id, name FROM portfolios WHERE user_id = $1 AND name = 'Micro Cap Winner Portfolio'`, [userId]);
        if (existingPortfolio.rows.length > 0) {
            ...
            for (const stock of stocksData) {
                await query(`INSERT INTO stocks (symbol, name, exchange, sector, industry, market_cap, tradable)
           VALUES ($1, $2, 'NASDAQ', 'Technology', 'Various', $3, true)
           ON CONFLICT (symbol) DO NOTHING`, [stock.symbol, stock.name, stock.marketCap]);
            }
            ...
            for (const symbol of stocksToAdd) {
                ...
                const result = await query(`INSERT INTO positions (portfolio_id, symbol, quantity, average_price, current_price, cost_basis, market_value, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
           ON CONFLICT (portfolio_id, symbol) DO NOTHING
           RETURNING id`, [portfolioId, symbol, quantity, avgPrice, avgPrice, costBasis, costBasis]);
      ...
      RETURNING id, name, enabled`, [
        ...
        for (const symbol of stocksToAdd) {
            ...
            const avgPrice = 10.00;
            const costBasis = quantity * avgPrice;
            await query(`INSERT INTO positions (portfolio_id, symbol, quantity, average_price, current_price, cost_basis, market_value, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
         ON CONFLICT (portfolio_id, symbol) DO NOTHING`, [portfolio.id, symbol, quantity, avgPrice, avgPrice, costBasis, costBasis]);
        }
...
//# sourceMappingURL=demo-migrate-user.handler.js.map