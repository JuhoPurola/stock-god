...
import { createPortfolioSchema, updatePortfolioSchema, } from '@stock-picker/shared';
 ...
 */
export async function listPortfolios(event) {
    try {
 ...
 * Get portfolio by ID
 */
export async function getPortfolio(event) {
    try {
 ...
 * Create portfolio
 */
export async function createPortfolio(event) {
    try {
        const userId = await getUserId(event);
 ...
 * Update portfolio
 */
export async function updatePortfolio(event) {
    try {
        const portfolioId = event.pathParameters?.id;
        if (!portfolioId) {
            return createApiResponse(400, { error: 'Portfolio ID required' });
 ...
 * Delete portfolio
 */
export async function deletePortfolio(event) {
    try {
        const portfolioId = event.pathParameters?.id;
        if (!portfolioId) {
            return createApiResponse(400, { error: 'Portfolio ID required' });
 ...
 */
export async function getPortfolioPositions(event) {
    try {
        const portfolioId = event.pathParameters?.id;
        if (!portfolioId) {
            return createApiResponse(400, { error: 'Portfolio ID required' });
 ...
 * Delete a position
 */
export async function deletePosition(event) {
    try {
        const portfolioId = event.pathParameters?.id;
        ...
        const position = await positionRepo.findByPortfolioAndSymbol(portfolioId, symbol);
        if (!position) {
...
//# sourceMappingURL=portfolios.handler.js.map