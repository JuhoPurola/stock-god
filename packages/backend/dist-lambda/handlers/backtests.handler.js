 ...
 */
export async function createBacktest(event) {
    try {
        ...
        setImmediate(async () => {
            try {
                const performance = await backtestService.runBacktest(backtest.id, backtest.config, strategy);
                ...
                await backtestRepo.updateStatus(backtest.id, BacktestStatus.FAILED, error.message);
            }
        });
 ...
 * Get backtest by ID
 */
export async function getBacktest(event) {
    try {
 ...
 */
export async function listBacktests(event) {
    try {
        const userId = await getUserId(event);
 ...
 */
export async function listPortfolioBacktests(event) {
    try {
 ...
 * Get backtest trades
 */
export async function getBacktestTrades(event) {
    try {
        const backtestId = getPathParam(event, 'id');
 ...
 * Delete backtest
 */
export async function deleteBacktest(event) {
    try {
        const backtestId = getPathParam(event, 'id');
        // Verify backtest exists
        const backtest = await backtestRepo.findById(backtestId);
...
//# sourceMappingURL=backtests.handler.js.map