...
export class StrategyService {
    ...
    async generateSignals(strategyId) {
        ...
        const signals = await strategy.generateSignals(strategyConfig.stockUniverse, (symbol) => this.getEvaluationContext(symbol));
     ...
     */
    async testStrategy(strategyId, symbol) {
        const strategyConfig = await this.strategyRepo.findByIdOrThrow(strategyId);
        // Create strategy instance
        const strategy = new MomentumStrategy(strategyConfig);
     ...
     */
    async getEvaluationContext(symbol) {
        // Get historical prices (last 200 days for indicators)
        const endDate = new Date();
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - 200);
        const historicalPrices = await this.stockRepo.getPriceHistory(symbol, startDate, endDate);
     ...
     */
    async executePortfolioStrategies(portfolioId) {
        const strategies = await this.strategyRepo.findEnabledByPortfolioId(portfolioId);
        const results = [];
