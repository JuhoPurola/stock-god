...
import { MomentumStrategy, SmallCapAlphaStrategy } from '@stock-picker/algorithm-engine';
/**
 * Backtest Engine
 */
export class BacktestService {
     ...
     */
    async runBacktest(backtestId, config, strategyConfig, options) {
        ...
        try {
            ...
            for (const date of tradingDays) {
                ...
                for (const signal of signals) {
                    await this.executeBacktestTrade(backtestId, portfolioState, signal, priceData, date, config, options?.skipDatabaseRecording);
                }
     ...
     */
    async executeBacktestTrade(backtestId, portfolioState, signal, priceData, date, config, skipDatabaseRecording) {
        ...
        if (side === OrderSide.BUY) {
            ...
            // Track trade in memory for performance calculations
            ...
            // Record trade to database (unless in demo mode)
            if (!skipDatabaseRecording) {
