...
export class TradingService {
  ...
  async executeTrade(
    ...
    if (side === OrderSide.BUY) {
      ...
      if (portfolio.cashBalance < estimatedCost) {
        throw new ValidationError(
          `Insufficient funds. Need $${estimatedCost.toFixed(2)}, have $${portfolio.cashBalance.toFixed(2)}`
        );
      }
    }
    ...
    if (side === OrderSide.SELL) {
      const position = await this.positionRepo.findByPortfolioAndSymbol(
        portfolioId,
        symbol
      );
      if (!position || position.quantity < quantity) {
        throw new ValidationError(
          `Insufficient position. Need ${quantity} shares, have ${position?.quantity || 0}`
        );
      }
    }
      ...
      try {
        ...
        switch (orderType) {
          case OrderType.MARKET:
            alpacaOrder = await alpacaClient.submitMarketOrder(
              symbol,
              side,
              quantity
            );
          ...
          case 'limit':
            ...
            alpacaOrder = await alpacaClient.submitLimitOrder(
              symbol,
              side,
              quantity,
              limitPrice
          ...
          case 'stop':
            ...
            alpacaOrder = await alpacaClient.submitStopOrder(
              symbol,
              side,
              quantity,
              stopPrice
          ...
          case 'stop_limit':
            ...
            alpacaOrder = await alpacaClient.submitStopLimitOrder(
              symbol,
              side,
              quantity,
              stopPrice,
        ...
        await this.tradeRepo.updateBrokerOrderId(
          trade.id,
          alpacaOrder.id,
          client
        );
        ...
        if (orderType === OrderType.MARKET) {
          ...
          const cashChange =
            side === 'buy' ? -quantity * filledPrice : quantity * filledPrice;
          await this.portfolioRepo.updateCashBalance(
            portfolioId,
            cashChange,
            client
          );
          ...
          await this.tradeRepo.updateStatus(
            trade.id,
            OrderStatus.FILLED,
            new Date(),
            client
  ...
  async executeSignals(
    ...
    for (const signal of signals) {
      try {
        ...
        const side: OrderSide = signal.type === 'BUY' ? OrderSide.BUY : OrderSide.SELL;
  ...
  private async updatePositionAfterTrade(
    ...
    return this.positionRepo.updateAfterTrade(
      portfolioId,
      symbol,
      quantityChange,
      price,
   ...
   */
  async syncPositions(portfolioId: string): Promise<void> {
    const portfolio = await this.portfolioRepo.findByIdOrThrow(portfolioId);
   ...
   */
  async checkOrderStatus(tradeId: string): Promise<Trade> {
    ...
    if (status !== trade.status) {
      const executedAt =
        alpacaOrder.filled_at ? new Date(alpacaOrder.filled_at) : undefined;
